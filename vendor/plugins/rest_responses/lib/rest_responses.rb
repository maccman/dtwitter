  ...
  extend self
   module ControllerMethods
     ...
     def responds(ob, options = {})
       ...
       respond_to do |wants|
         wants.xml  { 
           # See AdvancedToXml for reasons why we're not using 'render :xml'
           render :text => ob.to_xml(options), :content_type => Mime::XML, :status => status
         }
         ...
         wants.html { yield } if block_given?
       end
     end
